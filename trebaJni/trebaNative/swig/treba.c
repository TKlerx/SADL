...
void forward_print_path_to_file(struct trellis *trellis, struct wfsa *fsm, int obs_len, FILE *f) {
    int i, j, beststate;
    PROB bestprob;
    for (i = 0; i <= obs_len+1; i++) {
	if (i == obs_len)
	...
	fprintf(f,"%i", beststate);
	if (i < obs_len) fprintf(f," ");
    }
    fprintf(f,"\n");
}
...
void forward_fsm_to_file(struct wfsa *fsm, struct observations *o, int algorithm, char *filename) {
	...
	forward_prob = trellis_forward_fsm(trellis, obs->data, obs->size, fsm);
	if (algorithm == DECODE_FORWARD_PROB)
	    fprintf(f,"%.17g\t", output_convert(forward_prob));
	if (algorithm == LIKELIHOOD_FORWARD)
	    fprintf(f,"%.17g\n", output_convert(forward_prob));
	if (algorithm == DECODE_FORWARD_PROB || algorithm == DECODE_FORWARD) {
	    if (forward_prob > SMRZERO_LOG) {
		forward_print_path_to_file(trellis, fsm, obs->size,f);
	    } else {
		fprintf(f,"\n");
	    }
	}
...
void log1plus_taylor_init_wrapper(){
log1plus_taylor_init();
}

void log1plus_init_wrapper(){
log1plus_init();
}

void log1plus_free_wrapper(){
log1plus_free();
}
